...
df_flow = read.delim("./SimulatedFlow/NamsvatnDaily_RefFlow.txt", header = T, sep="\t")
df_flowmm<-df_flow
...
df_result<- pardata[,c("ParSet","R2","AbsACCD")]
...
for (i in 1:N){
  ...
  system('hbvpine  /t topol="D:/MyNTNU_April2019/NTE_Project/MyStudy/NamsGaugeDaily/ParameterFile/Parameter.top" input="D:/MyNTNU_April2019/NTE_Project/MyStudy/NamsGaugeDaily/InputData/NamsvatnObsDaily_1014_inp.dat" output="D:/MyNTNU_April2019/NTE_Project/MyStudy/NamsGaugeDaily/SimulatedFlow/NamsvatnGaugeDaily_Flow.txt" start=01.09.2010  00:00:00 stop=31.08.2014  00:00:00')
  print("SimFlowfile generated")
  
  df_sim <- read.delim("./SimulatedFlow/NamsvatnGaugeDaily_Flow.txt", header = T, sep="")
  Par<-pardata[,"ParSet"][i]
  name<- paste("SimFlow",as.character(Par), sep ="")
  df_flow[,name]<- df_sim[2:1462,"X1SIMDSCHRG"]
  
  Obs<- as.numeric(levels(df_sim[2:1462,"X1OBSRUNOFF"])[as.integer(df_sim[2:1462,"X1OBSRUNOFF"])])
  Sim<- as.numeric(levels(df_sim[2:1462,"X1SIMRUNOFF"])[as.integer(df_sim[2:1462,"X1SIMRUNOFF"])])
  RMSE<- round(rmse(Sim, Obs), digits=4)
  
  Obs_m3<- as.numeric(levels(df_sim[2:1462,"X1INOBSVAR"])[as.integer(df_sim[2:1462,"X1INOBSVAR"])])
  Sim_m3<- as.numeric(levels(df_sim[2:1462,"X1SIMDSCHRG"])[as.integer(df_sim[2:1462,"X1SIMDSCHRG"])])
  RMSE_m3<- round(rmse(Sim_m3, Obs_m3), digits=4)
  MAE_m3<- round(mae(Sim_m3, Obs_m3), digits=4)

  df_NS<-read.delim("./SimulatedFlow/NamsvatnGaugeDaily_Flow.inf", skip=1, header = F, sep="")
...
ResultFile<- "./SimulatedFlow/NamsvatnGaugeDaily_50SimFlow.txt"
write.table(df_flow, ResultFile, sep ="\t", row.names = TRUE, col.names = TRUE)


write.table(df_result,"./SimulatedFlow/NamsvatnGaugeDaily_50NS.txt" , sep ="\t", row.names = TRUE, col.names = TRUE)
print("ok")
