setwd("D:/MyNTNU_April2019/NTE_Project/MyStudy/NamsGaugeDaily")
getwd()
...
datfile <- "./ParameterAnalysis/MonteCarlo/NamsvatnGaugeDailyMC50.txt"
pardata <- read.delim(datfile, header = T, sep = "\t" )
N<- nrow(pardata)


df_result<- pardata[,c("ParSet","VerifyR2","AbsVerifyACCD")]
row.names(df_result)<- 1:50

newcols<-c("PresentR2","PresentACCD","RatioEvapPrec","RMSE","MAE","RMSE_m3", "MAE_m3")
...
for (i in 1:N){
  ...
  Dir<- "D:\\MyNTNU_April2019\\NTE_Project\\MyStudy\\NamsGaugeDaily\\ParameterFile\\"
  fil<- paste("Parameter",".top", sep="")
  filename<- paste(Dir,fil,sep="")
  sink(file=filename)
  ...
  system('hbvpine  /t topol="D:/MyNTNU_April2019/NTE_Project/MyStudy/NamsGaugeDaily/ParameterFile/Parameter.top" input="D:/MyNTNU_April2019/NTE_Project/MyStudy/NamsGaugeDaily/InputData/NamsvatnObsDaily_1416_inp.dat" output="D:/MyNTNU_April2019/NTE_Project/MyStudy/NamsGaugeDaily/SimulatedFlow/NamsGaugeDaily_Flow_Verify.txt" start=01.09.2014  00:00:00 stop=31.08.2016  00:00:00')
  print("SimFlowfile generated")
  
  df_sim <- read.delim("./SimulatedFlow/NamsGaugeDaily_Flow_Verify.txt", header = T, sep="")
  Par<-pardata[,"ParSet"][i]
  name<- paste("SimFlow",as.character(Par), sep ="")
  df_flow[,name]<- df_sim[2:732,"X1SIMDSCHRG"]
  
  Obs<- as.numeric(levels(df_sim[2:732,"X1OBSRUNOFF"])[as.integer(df_sim[2:732,"X1OBSRUNOFF"])])
  Sim<- as.numeric(levels(df_sim[2:732,"X1SIMRUNOFF"])[as.integer(df_sim[2:732,"X1SIMRUNOFF"])])
  RMSE<- round(rmse(Sim, Obs), digits=4)
  
  Obs_m3<- as.numeric(levels(df_sim[2:732,"X1INOBSVAR"])[as.integer(df_sim[2:732,"X1INOBSVAR"])])
  Sim_m3<- as.numeric(levels(df_sim[2:732,"X1SIMDSCHRG"])[as.integer(df_sim[2:732,"X1SIMDSCHRG"])])
  RMSE_m3<- round(rmse(Sim_m3, Obs_m3), digits=4)
  MAE_m3<- round(mae(Sim_m3, Obs_m3), digits=4)
  
  
  df_NS<-read.delim("./SimulatedFlow/NamsGaugeDaily_Flow_Verify.inf", skip=1, header = F, sep="")
  PresentR2<- round(df_NS[1,3], digits = 2)
  PresentACCD<-round(df_NS[2,3], digits = 2)
  RatioEvapPrec <- round(df_NS[8,3]/df_NS[7,3], digits=4)
  MAE<- round(df_NS[3,3], digits = 4)
  
  I<-as.character(i)
  df_result[I,][newcols] <- c(PresentR2,PresentACCD,RatioEvapPrec,RMSE,MAE,RMSE_m3, MAE_m3)
  
}
print("Loop finished")


ResultFile<- "./SimulatedFlow/NamsvatnGaugeDaily_50SimFlow_Verify.txt"
write.table(df_flow, ResultFile, sep ="\t", row.names = TRUE, col.names = TRUE)


write.table(df_result,"./SimulatedFlow/NamsvatnGaugeDaily_50NS_Verify.txt" , sep ="\t", row.names = TRUE, col.names = TRUE)
print("ok")
